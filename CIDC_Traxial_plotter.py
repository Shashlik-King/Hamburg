    ...
    for j in range(len(df_test['File_name'].values.tolist())):
        if int(df_test['Use_test'][j])==1:
           ...
           #----------------------------------------- WRITING PARAMETERS.INP --------------------------------------------#
            ...
            # write each test
            os.chdir(cwd+"\Post_process\TXD")
            
            def_real_param=pd.ExcelFile(df_test["File_name"].values.tolist()[j]+".xlsx").parse((pd.ExcelFile(df_test["File_name"].values.tolist()[j]+".xlsx")).sheet_names[1])
            ### there would be another function here reading the real style of furgo test 
            os.chdir(cwd2)
            
            # if def_real_param.values[9,1] == "True":
            #     var_values[0]= def_real_param.values[10,1] * def_real_param.values[2,1]  # E50_ref  = X0 *Dr
            #     var_values[1] = var_values[0]                                           # E50_ref  = E_oed_ref
            #     var_values[2]= var_values[0] * 3                                        # Eur_ref  = E50_ref *3
            #     var_values[3]= def_real_param.values[11,1] - def_real_param.values[12,1] * def_real_param.values[2,1]  # m = X1 - X2*Dr
            #     var_values[4]= def_real_param.values[13,1] + def_real_param.values[14,1] * def_real_param.values[2,1]  # Phi = Phi_cr + X3*Dr
            #     var_values[5]= 10.0 * def_real_param.values[2,1]
            #     var_values[6]= 1-np.sin(var_values[4]*3.1415/180)
            if def_real_param.values[9,1] == 1:
                var_values[0]= bound_dict["E50"][1]* def_real_param.values[2,1]  # E50_ref  = X0 *Dr
                var_values[1]= bound_dict["Eoed"][1]*def_real_param.values[2,1]                                           # E50_ref  = E_oed_ref
                var_values[2]= bound_dict["Eur"][1]*def_real_param.values[2,1]                                        # Eur_ref  = E50_ref *3
                var_values[3]= bound_dict["m"][0] - bound_dict["m"][1] * def_real_param.values[2,1] # m = X1 - X2*Dr
                var_values[4]= bound_dict["phi"][0] + bound_dict["phi"][1] * def_real_param.values[2,1]  # Phi = Phi_cr + X3*Dr
                var_values[5]= bound_dict["si"][1] * def_real_param.values[2,1]  # Si = coff*Dr
                var_values[6]= 1-np.sin(var_values[4]*3.1415/180)             
...
for n in range(len(alldata)):
    if alldata[n].test_type=="TXD":
        ...
        if df["involvment-EpsV-eps1"][indx]!=-1:
            if col ==20:
                line1=plt.plot(alldata[n].X, alldata[n].Z,label=alldata[n].test_name+ " Real test" , color=colors[col],marker='o')
            else:
                line1=plt.plot(alldata[n].X, -alldata[n].Z,label=alldata[n].test_name+ " Real test" , color=colors[col],marker='o')
                
            line2=plt.plot(eps1,epsV,label=alldata[n].test_name + " Simulated test" , color=colors[col])
            #ax1.set_xlim(0,0.01)
