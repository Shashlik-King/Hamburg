...
class real_test():    
    def __init__(self,test_type,test_name,X,Y):
        self.X=np.array(X)
        self.Y=np.array(Y)
...
def forwardmodel(df_test,xl,cwd2,df_param,Constitutive_model,Nstate_var):
    ...
    ##############################################################################################################        
            ...
            alldata.append(real_test(df_test['Test_type'][j],test_name[0],def_real[def_real.columns[0]],def_real[def_real.columns[1]]))
