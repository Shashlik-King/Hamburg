...
uiwait(msgbox("Importing Excel-data with manually defined ranges in function 'XLSXread'. Ensure these are defined correctly"));
for i = 1:length(listing)
    num_data = xlsread(fullfile(Folder.Data.CPT,char(listingCheck)), char(listing(i))); % Reading depth, qt, fs, u2
    %num_data(:,3) = [];
...
if ~isfield(data.postprocess.(char(data.nameSave{i})),'qt') && settings.CALC.qt
    data.postprocess.(data.nameSave{i}).qt = data.postprocess.(data.nameSave{i}).qc + data.postprocess.(data.nameSave{i}).u2.*(1-data.postprocess.(data.nameSave{i}).CAR);
end
end
