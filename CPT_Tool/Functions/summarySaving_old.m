...
if exist(fullfile(Folder.Output,settings.Files.Output),'file')                                 % Check if summary file exist
    ...
    output.Matrix = [{'Location'},{'Layer'},{'Top level'},{'Bottom level'},{'Unit'},{'Rob. zone distribution 1'},{'Rob. Zone 1'},{'SBT 1'},{'Rob. zone distribution 2'},{'Rob. Zone 2'},{'SBT 2'},{'Preparer'},{'Date'},{'QA´er'},{'Date'}; {'[-]'},{'[No.]'},{'[mBSL]'},{'[mBSL]'},{'[-]'},{'[%]'},{'[-]'},{'[-]'},{'[%]'},{'[-]'},{'[-]'},{'[Initials]'},{'[dd-mmm-yy]'},{'[Initials]'},{'[dd-mmm-yy]'}];
    fileExist = 0;
end
...
if settings.QAmode      % If in QA mode, the QA'er columns should be updated
    ...
    if isempty(indexDelete)
        ...
        InitialVector = [output.ObsoleteMatrix(indexDelete(1),12:13)];                 % If any previous data is found, they are deleted
...
output.Addition = cell(length(data.postprocess.(data.nameSave{i}).layerLevel)+1,15);    % Define cell array dimensions for new data
...
for j = 1:length(data.postprocess.(data.nameSave{i}).layerLevel)+1
    output.Addition(j,:) = [data.nameSave{i},{j},layer(j), layer(j+1), {'ToBeUpdated'},data.postprocess.(data.nameSave{i}).zone.LocationSum{1}(j,1:end-1), InitialVector, QAvector];
end
