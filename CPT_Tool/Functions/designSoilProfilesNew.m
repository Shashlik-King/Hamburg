        ...
        for k = 1:kMax  % Loop over different methodologies for same parameter
            ...
            for ii=1:size(data.postprocess.(data.nameSave{i}).indexLayer,1) % Loop over each layer
                ...
                for JJ = 1:steps
                    ...
                    dat = rmmissing(rmoutliers(data.postprocess.(data.nameSave{i}).(parameters{j}).values(idxSubLayer,k),'percentiles',inter_conf1));
                    SDval = sqrt(sum((dat-mean(dat)).^2)/(length(dat)-1));
                    valLB(JJ) = round(mean(dat-SDval),6);
                    valUB(JJ) = round(mean(dat+SDval),6);

                    y(JJ) = val(JJ);  % Value representative for depth

                    yLB(JJ) = valLB(JJ);  % Value representative for depth
                    yUB(JJ) = valUB(JJ);  % Value representative for depth
                end
                ...
                if steps==1     % If only 1 point available for the layer
                    ...
                    if isempty(y)
                        ...
                        plotDLval.(parameters{j}).TB{ii,k}.BE = [yLB; yLB];
                    end
                    if isempty(y)
                        plotDLval.(parameters{j}).TB{ii,k}.CLB = [0; 0];
                    else
                        plotDLval.(parameters{j}).TB{ii,k}.CUB = [yUB; yUB];
                    end
                else
%                     p = polyfit(x,y,1);
...
%                     plotDLval.(parameters{j}).poyVal{ii,k} = p; % Constants for linear fit
%                     f = polyval(p,x(:));
% %                     T = table(x,yLB,f,yLB-f,'VariableNames',{'X','Y','Fit','FitError'});
%                     plotDLval.(parameters{j}).TB{ii,k}.CLB       = (layerLevels(ii:ii+1)*p(1)+p(2));     % Top bottom values for plot
% 
%                     p = polyfit(x,yUB,1);
%                     plotDLval.(parameters{j}).poyVal{ii,k} = p; % Constants for linear fit
%                     f = polyval(p,x(:));
% %                     T = table(x,yUB,f,yUB-f,'VariableNames',{'X','Y','Fit','FitError'});
%                     plotDLval.(parameters{j}).TB{ii,k}.CUB       = (layerLevels(ii:ii+1)*p(1)+p(2));     % Top bottom values for plot
...
%         plotOutput_addition_wEst(settings,Folder,values,data,i,1,plotDLval)
        plotOutput_addition_wEst(settings,Folder,values,data,i,0,plotDLval)
    catch ME
        test = 1;
    end
