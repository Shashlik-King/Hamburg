function data = ZoneDetermination(data, i, Method)
%%%-------------------------------------------------------------------------%%%
...
if Method == 2 || Method == 12
    ...
    xvalue = data.postprocess.(data.nameSave{i}).Fr;
    yvalue = data.postprocess.(data.nameSave{i}).Qt;
elseif Method == 3
    ...
    xvalue = data.postprocess.(data.nameSave{i}).Bq;
    yvalue = data.postprocess.(data.nameSave{i}).Qt;
elseif Method == 4
    ...
    xvalue = data.postprocess.(data.nameSave{i}).Rf;
    yvalue = data.postprocess.(data.nameSave{i}).qc;
end
...
if Method == 1
    ...
    zone_save = NaN(length(data.postprocess.(data.nameSave{i}).Ic),1);
    for j = 1:length(ZoneIcInterval)
        if j == 1
            indexSBT = data.postprocess.(data.nameSave{i}).Ic>=ZoneIcInterval(j,3);
...
elseif Method == 5
    ...
    Ic = data.postprocess.(data.nameSave{i}).Ic;
    Qtn = data.postprocess.(data.nameSave{i}).Qtn;
...
if Method == 12     % COWI refined Rob1990FrQt method with Ic value
    ...
    for j = 1:length(zone_save)
        if zone_save(j) > 2.5 && zone_save(j) < 6.5
            if data.postprocess.(data.nameSave{i}).Ic(j) > Limits(zone_save(j))
...
data.postprocess.(data.nameSave{i}).ZoneData.(methodsName{Method,2}) = zone_save;     % Save the zone index in the postprocessed data structure